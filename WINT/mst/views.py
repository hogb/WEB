...
# <form id="@" action="/kr_stock/" method="POST" onclick="document.getElementById('@').submit();">
...
kind_lam = \
    {
        ...
        '반도체대분류':('{background-color:D2042D; color:White; font-weight:bold}',lambda x: f'<div class="반도체대분류">'+str(x)+'</div>' if (str(sec_dict.get(str(str(x).replace('<','').split('form id="')[-1].split('"')[0]),'-')).count('반도')>0)  else x)
        ,'은행대분류':('{background-color:D3D3D3; color:Black; font-weight:bold}',lambda x: f'<div class="은행대분류">'+str(x)+'</div>' if (str(sec_dict.get(str(str(x).replace('<','').split('form id="')[-1].split('"')[0]),'-')).count('은행')>0)  else x)
        ,'전기차대분류':('{background-color:00FFFF; color:Black; font-weight:bold}',lambda x: f'<div class="전기차대분류">'+str(x)+'</div>' if (str(sec_dict.get(str(str(x).replace('<','').split('form id="')[-1].split('"')[0]),'-')).count('전기')>0)  else x)
        ,'에너지대분류':('{background-color:8B4000; color:White; font-weight:bold}',lambda x: f'<div class="에너지대분류">'+str(x)+'</div>' if (str(sec_dict.get(str(str(x).replace('<','').split('form id="')[-1].split('"')[0]),'-')).count('에너지')>0)  else x)
        ,'PCB대분류':('{background-color:0015FF; color:White; font-weight:bold}',lambda x: f'<div class="PCB대분류">'+str(x)+'</div>' if (str(sec_dict.get(str(str(x).replace('<','').split('form id="')[-1].split('"')[0]),'-')).count('PCB')>0)  else x)
        ,'항공대분류':('{background-color:00FF00; color:Black; font-weight:bold}',lambda x: f'<div class="항공대분류">'+str(x)+'</div>' if (str(sec_dict.get(str(str(x).replace('<','').split('form id="')[-1].split('"')[0]),'-')).count('항공')>0)  else x)
        
        ,'등락상':('{background-color:F08080; color:Black; font-weight:bold}',lambda x: f'<div class="등락상">'+str(x)+'</div>' if ((type(x) != type('s')) and ( x >= 1))  else x)
        ,'등락상상':('{background-color:FF0000; color:White; font-weight:bold}',lambda x: f'<div class="등락상상">'+str(x)+'</div>' if ((type(x) != type('s')) and ( x >= 3))  else x)
        ,'등락상상상':('{background-color:C11B17; color:White; font-weight:bold}',lambda x: f'<div class="등락상상상">'+str(x)+'</div>' if ((type(x) != type('s')) and ( x >= 7))  else x)
        ...
        ,'거율상상':('{background-color:52D017; color:000000; font-weight:bold}',lambda x: f'<div class="거율상상">'+str(x)+'</div>' if ((type(x) != type('s')) and ( x >= 16))  else x)
...
def index(request):
    ...
    tgs += '''
    <form action='/kr_stock/' method="post">
        <input type='text' name='stock_name' style="width:28rem; font-size:4rem; height:5rem" >
        <input type='submit' value='검색' style="width:8rem; font-size:4rem; height:5rem" >
    
    </form><br><br>
    '''


    for k,v in zip(mdict.keys(),mdict.values()):
        tgs += f'<a href="/{k}">{ v }</a>'
        tgs += '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp'
    ...
    <h1><a href="https://m.search.naver.com/search.naver?query=%EC%84%9C%EC%9A%B8+%EB%82%A0%EC%94%A8&where=m&sm=mob_sug.asiw&acq=%EC%84%9C%EC%9A%B8+&acr=1&qdt=0" class="타이틀">
    <br><br>서울날씨
    </a></h1>
    ...
    index_html = utml + con_send + dtml + etctml
...
def m4(request):
    ...
    for i in '7일_상승변등이후,20일_상승변등이후,60일_상승변등이후,7일_변등평,20일_변등평,60일_변등평,7일_변경율평,20일_변경율평,60일_변경율평,7일_카운트,20일_카운트,60일_카운트'.split(','):
    ...
    <form action='/m4/' method="post">
        ...
        <select name="note_kind" style="font-size:1.5rem;text-align: center ">
                <option value="{'원본'}">  {'원본'}  </option>'
                <option value="{'오늘'}">  {'오늘'}  </option>'
                <option value="{'변경'}">  {'변경'}  </option>'
                <option value="{'오늘변경'}">  {'오늘변경'}  </option>'
		</select>
    ...
    if request.method == "GET":
        note_html,la_time = NOTE_today('-','-')
        la_time = f'<div class="la_time" onclick="Hello();"><h1>{la_time} </h1></div>'
        m4_html = utml + la_time +comp +'<br><br>'+comp1 + comp2 +'<br><br>'+ comp3  +'<br><br>'+  note_html + dtml
        return HttpResponse(f'{m4_html}')

    elif request.method == "POST":
        note_html,la_time = NOTE_today(request.POST['note_kind'],request.POST['note_sort'])
        la_time = f'<div class="la_time" onclick="Hello();"><h1>{la_time} </h1></div>'

        la_page = f"<br><h1>{request.POST['note_kind']}  {request.POST['note_sort']}</h1>"

        #m4_html = la_time +comp + comp2 + comp3 + comp4 +'<br>'+ utml + note_html + dtml
        m4_html = utml + la_time +comp +'<br><br>'+comp1 + comp2 +'<br><br>'+ comp3 +'<br><br>' + la_page +'<br><br>'+  note_html + dtml
        return HttpResponse(f'{m4_html}')
    else:
        1
...
def m7(request):
    ...
    news_html,la_time = plog_sec()
    utml = basic_uml(kind_html)
    dtml = basic_dml()
    la_time = f'<div class="la_time"><h1>{la_time}</h1></div>'

    m7_html = la_time + utml + news_html + dtml

    return HttpResponse(f'{m7_html}')
...
def plog():
    ...
    k_dict = {
        ...
        '잠정상':[ ['잠정외기%','예대'], [False,False] ],
        '잠정하':[ ['잠정외기%','예대'], [True,True] ],
        ...
        '7수':['7일_외기법_시총대비_외기법', False],
        '30수':['30일_외기법_시총대비_외기법', False],

        '롱총점':[ ['총점_J','30수종'], [False,False] ],
        '숏총점':[ ['총점_J','30수종'], [True,True] ],
    ...
    tdf['시총조'] = np.round(tdf['시총']/10000,1)
    ...
    tgtg = {
        ...
        '14수종':2,'30수종':2,'수급7일':0.5,'수급30일':0.5,'세투상':0,
            ...
            '컨상':0,'컨하':0,'롱총점':0,'숏총점':0,'발표':0,'바텀':0.1
            }
    cuscol = ['종목명+종토링크','ST_등락','섹터_B','총점_J','실적이슈','종목명_P','1고현폭','1현재폭','예대','등락율_M','체결강도_M','체강구간_끝','외국계%','잠정외인%','잠정기관%','프로그램%','발표간격','시총조','필터점수']
    ...
    #
    org_list = ['종목명+종토링크','ST_등락','섹터_B','총점_J','실적이슈','종목명_P','1고현폭','1현재폭'
        ,'예대','등락율_M','체결강도_M','체강구간_끝','7일평단대비','30일평단대비','외국계%','잠정외인%','잠정기관%','프로그램%','7일외기법%','30일외기법%'
        ,'7일_외기법_시총대비_외기법','30일_외기법_시총대비_외기법','발표간격','섹터등급_B','30STO표','시총조'
        ]
...
def plog_sec():
    ...
    org_list = ['종목명+종토링크','섹터_B','총점_J','실적이슈','종목명_P','1고현폭','1현재폭'
        ,'예대','ST_등락','등락율_M','체결강도_M','체강구간_끝','7일평단대비','30일평단대비','외국계%','잠정외인%','잠정기관%','프로그램%','7일외기법%','30일외기법%'
        ,'7일_외기법_시총대비_외기법','30일_외기법_시총대비_외기법','발표간격','섹터등급_B','30STO표','시총조'
                ]


    tdf = tdf[~tdf['섹터_B'].str.contains('|'.join(['제외','제외2','몰라','기타','코로나','후후','상장','가구']))]


    ST = tdf[['섹터_B','ST_등락','ST_수']].sort_values(by='ST_등락', ascending=False)
    ST = ST[ST['ST_수'] >= 2]
    ST = ST.drop_duplicates(keep='first')

    secdetail_dict = {}
    for i in ST['섹터_B'].unique():
        temp_sec = tdf[tdf['섹터_B'] == i]
        temp_sec['절대_등락율_M'] = np.abs(temp_sec['등락율_M'])
        temp_sec = temp_sec.sort_values(by='절대_등락율_M', ascending=False).reset_index(drop=True)
        #print(temp_sec)
        secdetail = ''
        ...
        for j in temp_sec.index:
            dcnt += 1
            if dcnt <= min(20,len(temp_sec)):
                temptail = ''
                if temp_sec.at[j,'등락율_M'] >= 1:
                    temptail += '<b>'+str(temp_sec.at[j,'종목명_P'])[:10] + '<font color="red">' + str(temp_sec.at[j,'등락율_M']) + '</font></b>' + '<font color="white">' + '호..' + '</font>'
                elif temp_sec.at[j,'등락율_M'] <= -1:
                    temptail += '<b>'+str(temp_sec.at[j,'종목명_P'])[:10] + '<font color="blue">' + str(temp_sec.at[j,'등락율_M']) + '</font></b>' + '<font color="white">' + '호..' + '</font>'
                else:
                    temptail += str(temp_sec.at[j,'종목명_P'])[:10] + '<font color="black">' + str(temp_sec.at[j,'등락율_M']) + '</font>' + '<font color="white">' + '호..' + '</font>'

                # if temp_sec.at[j,'예대'] >= 100:
                #     temptail = '<font size="2rem">' + temptail + '</font>'
                # else:
                #     temptail = '<font size="1rem">' + temptail + '</font>'
    ...
    temp_ysec = ST
    temp_ysec['디테일'] = temp_ysec['섹터_B'].apply(lambda x: secdetail_dict.get(x,'-'))

    if len(temp_ysec)>-1:

        temp_ysec = df_style(temp_ysec)

        temp_html = str(temp_ysec.to_html(index=False,table_id='plog_sec',escape=False))

    plog_html += temp_html
    return [plog_html,last_time]


def NOTE_today(note_kind='-',note_sort='-'):
#def NOTE_today(ctr=9999,chg=0,plus=0,note_sort='-'):
    ...
    if note_kind == '-':
        ctr=9999; chg=0; plus=0
    elif note_kind == '오늘':
        ctr=0; chg=0; plus=0
    elif note_kind == '변경':
        ctr=9999; chg=1; plus=3
    elif note_kind == '오늘변경':
        ctr=0; chg=1; plus=0
    elif note_kind == '원본':
        ctr=9999; chg=0; plus=0
    ...
    try:
        ...
        TODAY = TODAY.sort_values(by='순서',ascending=False)
    except:
    ...
    col_list = '7일_변등평,종목명,등락율_M,회사명,베스트,변등,변경율,REP간,의견변경,주가변경,업사,투자의견,날짜간격,오늘,1현재폭,1고저폭,20현재폭,20고저폭,90현재폭,90고저폭,REP사이날짜,종목명+종토링크,제목,날짜,서브날짜,목표주가,서브목표주가,그날가격,서브그날가격,다음key,서브key,총점'.split(',')
    #,7일_총합,1일_총합

    if note_sort not in ['-','7일_변등평']:
        TODAY = TODAY[[note_sort]+col_list]
    else:
        TODAY = TODAY[['7일_상승변등이후']+col_list]
...
def news():
    ...
    n1 = pd.merge(n1,pg[['종목명_P','등락율_M','섹터_B']].add_suffix('_plog'),left_on='종목명_N',right_on='종목명_P_plog', how='left')
    ...
    n1['종목명+종토뉴스링크'] = n1.apply(lambda x :  f"<a href='{str(x['종토링크_mst'])}'>{'토'}</a>" , axis=1)
    n1['종목명+종토뉴스링크'] = n1['종목명+종토뉴스링크'].str.replace('board','news')

    n1 = n1[['섹터_B_plog','등락율_M_plog','종목명_N','종목명+종토뉴스링크','제목','뉴스간격','정보제공','날짜']]
    n1['섹터_B_plog'] = n1['섹터_B_plog'].astype('str')
...
def kr_stock(request):
    ...
    링딕 = mp[['종목명_P','링크','종토링크','fn링크']].set_index('종목명_P').T.to_dict()
    ...
    if request.method == "GET":
        ...
        comp = f'''
    <form action='/m44/' method="post">
            <input type='hidden' name='report' value='{str(request.POST['stock_name'])+',150'+',10'}'>
    ...
    '''
        ...
        fmp = mp[mp['종목명_P'] == request.POST['stock_name']]

        if len(fmp) == 1:
            test = list(fmp['종목명_P'])[0]

        else:
            fmp = mp[(mp['종목명_P'].str.contains(request.POST['stock_name']))|(mp['별명_B'] == request.POST['stock_name'])]

            test = list(fmp['종목명_P'])[0]


        #test = request.POST['stock_name']
        ...
        if len(tlist) <=0 :
            ...
            reshtml += f"""
                    ...
                    &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                    <a class="rem2" href="{링딕.get(str(test)).get('링크')}""> {'다트'}  </a>
