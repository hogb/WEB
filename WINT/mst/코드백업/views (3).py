...
def m4(request):
    ...
    comp2 = '''<button type="button" style="width:12rem; font-size:2rem;height:3rem" 
    ...
    <form action='/m44/' method="post">
            <input type='text' name='report' style="height:3rem; width:6rem; font-size:2rem; text-align: center" />
            <input type='submit' value='레포트' style="height:3rem; width:6rem; font-size:2rem; text-align: center" />
    </form>
    <br>
    '''
    comp3 = '''
    ...
    if request.method == "GET":
        ...
        m4_html = la_time +comp +'<br>'+ comp2 + comp3 + comp4 +'<br><br><br>'+ utml + note_html + dtml
        return HttpResponse(f'{m4_html}')

    elif request.method == "POST":
        note_html,la_time = NOTE_today(int(request.POST['note_ctr']),int(request.POST['note_chg']),int(request.POST['note_plus']))
        la_time = f'<div class="la_time" onclick="Hello();"><h1>{la_time} </h1></div>'
...
def NOTE_today(ctr=9999,chg=0,plus=0):
    ...
    if chg == 1:
        rdf = rdf[(~rdf['의견변경'].isin(['유지','없']))|(~rdf['주가변경'].isin(['유지','없']))]
...
def kr_stock(request):
    ...
    '''
        ...
        if len(tlist) <=0 :
            ...
            reshtml += f"""
                    <div class="rem2"><a href="{링딕.get(str(test)).get('fn링크')}"> {'fn링크  '}  </a></div>
                    <div class="rem2"><a href="{링딕.get(str(test)).get('종토링크')}""> {'종토  '}  </a></div><br>
                    """
...
def reply2_list(request):
    ...
    if request.method == "GET":
        ...
        for sx in tgdf['종목']:
            ...
            jm = rf"C:\Users\Jinu\PycharmProjects\CYB\JAMU\save\SUM"
            fd = Path(__file__).resolve().parent.parent
