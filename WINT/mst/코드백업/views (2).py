...
def basic_uml(cont=None,cont2=None):
    ...
    .넓 {{padding-left:{th_padding*2}rem ; white-space: nowrap}}
...
def m0(request):
    ...
    <body>
        <form action='/reply2_list/' method="post"><br>
            ...
            <br><textarea name='cond_kind' rows={1} cols={int(col/3)}>1</textarea><input type='submit' value='리스트추출'>
...
def m1(request):
    ...
    <body>
        <form action='/reply2/' method="post"><br>
            ...
            <br><textarea name='cond_kind' rows={1} cols={int(col/3)}>1</textarea><input type='submit' value='추출'>
...
def m4(request):
    ...
    comp = '<button type="button" onclick="location.href=\'https://comp.fnguide.com/SVO2/ASP/SVD_Report_Summary.asp?pGB=1&gicode=A005930&cID=&MenuYn=Y&ReportGB=&NewMenuID=901&stkGb=701\'">요약써머리</button>'
    comp2 = '''
    ...
    m4_html = la_time +comp + comp2+'<br>'+ utml + note_html + dtml
...
def m44(request):
    ...
    if request.method == "GET":
        ...
        fname = (f'report_"+{str(datetime.now()).split(".")[0]+"_"+str(stox)}')
...
def bot3(mrow=100):
    ...
    col = ['알람@','등락','ST_등락','종목명_P','링크','프로그램%','체강구간_끝','체강','1고현폭','1현재폭','5고현폭','5현재폭','20고현폭','20현재폭','90고현폭','90현재폭','특표']
    col = ['알람@','등락','ST_등락','종목명_P','링크','프로그램%','체강구간_끝','체강','1고현폭','1현재폭','특표']
    tcol = ['잠정외인%','잠정기관%','잠정법인%','총점_J','실적이슈','발표간격','7일외기법%','7일평단대비','30일외기법%','30일평단대비','섹터_B','섹터등급_B','진대','예대','필터','양주체들_SE','음주체들_SE']
...
def plog():
    ...
    fil_df = tdf[tdf['필터']!=''].sort_values(by='필터점수', ascending=False).set_index('종목명_P',drop=True).head(120)
    ...
    new_df = new_df.reset_index(drop=False).fillna('')
    new_df['필터점수'] = np.round(new_df['필터점수'].astype(float),1)

    newdf = df_style(new_df)

    newdfml = str(newdf.to_html(index=False,table_id='plog newdf',escape=False))
    ...
    #
    org_list = ['1고현폭','1현재폭','예대','종목명_P','등락율_M','체결강도_M','체강구간_끝','7일평단대비','30일평단대비','잠정외기법%','프로그램%','7일외기법%','30일외기법%'
        ,'7일_외기법_시총대비_외기법','30일_외기법_시총대비_외기법','총점_J','실적이슈','발표간격','섹터_B','섹터등급_B','30STO표'
        ]
    ...
    for ky in list(k_dict.keys()): #reversed(list(k_dict.keys()))
        ...
        tempml = str(tempdf.to_html(index=False,table_id='plog '+str(ky),escape=False))
...
def NOTE_today():
    ...
    col_list = '종목명,등락율_M,회사명,베스트,7일_변등평,변등,변경율,REP간,의견변경,주가변경,업사,투자의견,날짜간격,오늘,1현재폭,1고저폭,20현재폭,20고저폭,90현재폭,90고저폭,REP사이날짜,종목명링크,제목,날짜,서브날짜,목표주가,서브목표주가,그날가격,서브그날가격,다음key,서브key,7일_총합,1일_총합,총점,7일_상승변등이후'.split(',')
...
def news():
    ...
    nt['링크'] = nt['링크'].apply(lambda x :  f"<a href='{str(x)}'>{'링크'}</a>" )
    nt = nt[['제목','링크','날짜','시간','표시','구분']]
...
def kr_stock(request):
    ...
    링딕 = mp[['종목명_P','종토링크','fn링크']].head(5).set_index('종목명_P').T.to_dict()
    ...
    if request.method == "GET":
        ...
        if len(tlist) <=0 :
            ...
            for x in fls:
                if str(x).count(tams)>0:
                    ...
                    reshtml = f"""
                    <div class="rem2"><a href="{링딕.get('삼성전자').get('fn링크')}"> {'fn링크  '}  </a></div>
                    <div class="rem2"><a href="{링딕.get('삼성전자').get('종토링크')}""> {'종토  '}  </a></div><br>
                    """
        ...
        return HttpResponse(utml + reshtml + dtml)
...
def reply2_list(request):
    ...
    if request.method == "GET":
        ...
        for sx in tgdf['종목']:
            ...
            jm = r"C:\Users\Jinu\PycharmProjects\CYB\JAMU\save\SUM"
            fd = Path(__file__).resolve().parent.parent
            ...
            if len(tlist) <=0 :
                ...
                for x in fls:
                    if str(x).count(tams)>0:
                        try:ressrc = os.path.join('static','SUM',str(x))
                        except:ressrc = '1'
